...
user_churn_periods_cc as (
    ...
    select
        ...
        case when  (dateadd(day,+1,max(last_active_day))<='2017-06-01' and dateadd(day,+28,max(last_active_day))>'2017-06-01') then '2017-06-01'
            else dateadd(day,+28,max(last_active_day)) end as last_churn_day
           from user_active_periods_c
       group by user_id
...
user_churn_periods_c as (
    select * from user_churn_periods_cc
    where first_churn_day<='2017-06-01' and last_churn_day<='2017-06-01'
),
...
select  user_id,
        avg(churn_duration) as avg_churn_duration,
        count (churn_duration) as num_churn_periods
from (
         select user_id,
                datediff(day, first_churn_day, last_churn_day) + 1 as churn_duration
         from user_churn_periods_c
     )
group by user_id