...
create view sleal.user_days as

--find activity periods, day with tasks until 28 days after
with user_active_periods as (
     select user_id,
            case when sum(sum_tasks_used)>=1 then date end as first_active_day, --day with tasks
            case when sum(sum_tasks_used)>=1 then dateadd(day,27,date) end as last_active_day --after 28 days
    from source_data.tasks_used_da
    group by user_id, date
),

--activity periods have overlapping dates, so it's needed to pack data
-- condensing time periods for active periods, per user
active_period as(
select user_id, first_active_day as ts, +1 as type, 1 as sub --day with tasks
  from user_active_periods
union all
select user_id, dateadd(day, +1, last_active_day) as ts, -1 as type, 0 as sub --day it stops being active (after 29 days)
  from user_active_periods),
...
user_churn_periods as (
    select
        user_id,
        dateadd(day, +1, last_active_day) as first_churn_day, --one day after last active day
        dateadd(day, +28, last_active_day) as last_churn_day --churn lasts 28 days
...
--find overlap
--ensure all days with activity are removed from churn periods
...
--add final churn period (after last activity, 28 days after last task)
...
select ts.user_id,
       count(distinct num_accounts) as num_accounts,
       sum(sum_tasks_used) as sum_tasks,
       sum(ischurn) as days_churn,
       sum(isactive) as days_active
from user_status_full_ts ts
left join totals t on t.user_id=ts.user_id and ts.date=t.date
group by ts.user_id
